...
public class TileBoardView extends RelativeLayout implements OnTouchListener {
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_DOWN: // Begin
                // Save last move x,y
                this.lastMoveX = (int) event.getX();
                this.lastMoveY = (int) event.getY();

                // Save dragged tile
                this.assignDraggedTileAtPoint(new Point((int) event.getX(), (int) event.getY()));
                break;
    ...
    public void playWithImage(Bitmap image, int xSize, int ySize, int screenWidth) {
        ...
        for (int j = 1; j <= this.board.getYSize(); j++) {
            for (int i = 1; i <= this.board.getXSize(); i++) {
                ...
                RelativeLayout.LayoutParams params =
                		new RelativeLayout.LayoutParams(this.tileWidth, this.tileHeight);
    ...
    private void tileWasMoved() {
        if (this.board.isAllTilesCorrect()) {
            ...
            // Get tile by coordinate
            int value = this.board.tileAtCoordinate(new Point(this.board.getXSize(), this.board.getYSize()));

            ImageView tileView = this.tiles.get(this.tiles.size() - 1);

            // setting image position
            RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(this.tileWidth, this.tileHeight);
    ...
    private Point coordinateFromPoint(Point point) {
        ...
        return new Point((point.x / this.tileWidth) + 1, (point.y / this.tileHeight) + 1);
    }

    private void assignDraggedTileAtPoint(Point position) {
        Point coord = this.coordinateFromPoint(position);

        this.movingTilePoint = coord;

        // Cannot move this tile
        if (!this.board.canMoveTile(coord)) {
            this.draggedDirection = 0;
            this.draggedTile = null;
            return;
        }

        if (this.board.tileAtCoordinate(new Point(coord.x, coord.y - 1)) == 0) {
            this.draggedDirection = Direction.UP.val();
        } else if (this.board.tileAtCoordinate(new Point(coord.x + 1, coord.y)) == 0) {
            this.draggedDirection = Direction.RIGHT.val();
        } else if (this.board.tileAtCoordinate(new Point(coord.x, coord.y + 1)) == 0) {
    ...
    private boolean rectContainsPoint(ImageView rect, Point point) {
        return (rect.getX() < point.x && rect.getY() < point.y
                && rect.getX() + rect.getWidth() > point.x && rect.getY() + rect.getHeight() > point.y);
    }

    private void movingDraggedTile(Point translationPoint) {
        int x = 0, y = 0;

        //Log.d("TranslationPoint", "x=" + translationPoint.x + "/" + "y=" + translationPoint.y);
    ...
    private void moveTile(ImageView tile, int direction, Point tilePoint) {
        ...
        switch (direction) {
            case 1: deltaY = -1; break; /* UP */
            case 2: deltaX =  1; break; /* RIGHT */
            case 3: deltaY =  1; break; /* DOWN */
            case 4: deltaX = -1; break; /* LEFT */
            default: break;
        }
    ...
    private void snapDraggedTile() {
        ...
        if (this.draggedDirection == Direction.UP.val()) {
            if (this.totalTranslateY < -(this.tileHeight / 2)) {
                this.moveTile(this.draggedTile, this.draggedDirection, this.movingTilePoint);
            } else {
